...
public class Heap<E> extends CompleteBinaryTree<E> {
	...
	/**
	 * Method to add a new element to the heap. Adds a new element to the heap. The
	 * heap is assumed to be a min-heap. Always add the new element at the first
	 * position available in the tree. Just add to the position list.size() in the
	 * array list. After the new element is added, apply upheap to guarantee that
	 * the current content (which is a heap), plus the new element, continue
	...
	/**
	 * Moves upward, as needed, in the path from p to root, the element at p with
	 * the goal of maintaining the heep property after the new element is added.
	 * Potentially, this goes upward, all the way from p to root, or until the value
	 * initially in p reached a level in which the key at its parent is less than
	 * the key initially at p.
	 * 
	 * @param p
	 */
